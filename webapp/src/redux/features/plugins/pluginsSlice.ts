...
export const pluginsState = createSlice({
  ...
  reducers: {
    connectPlugin: (
      state: PluginsState,
      action: PayloadAction<EnablePluginPayload>
    ) => {
      ...
      switch (action.payload.plugin) {
        case BuiltInPlugins.Jira:
          authData = `${action.payload.email as string}:${
...
export const { connectPlugin, disconnectPlugin, addPlugin } =
  pluginsState.actions;
